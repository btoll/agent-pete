...
func (a *Agent) CallTool(toolCall api.ToolCall) (string, error) {
	...
	if t, found := tool.Tools[funcName]; found {
		...
		case "GetWeather":
			if location, found := toolCall.Function.Arguments["location"]; found {
				return tool.GetWeather(location.(string))
			}
			return "", errors.New("argument `location` not found")
		case "ReadFile":
...
func (a *Agent) MakeRequest() (*api.Request, error) {
	...
	dbMessages, err := a.GetPreviousMessages(10)
	if err != nil {
		return nil, err
	}
